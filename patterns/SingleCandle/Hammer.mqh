#ifndef PATTERNS_HAMMER_MQH
#define PATTERNS_HAMMER_MQH

// Hammer.mqh
// Bullish-leaning "Hammer" implemented with the OOP strategy base (PatternBase.mqh).
// Definition (typical): small body near the HIGH, long lower shadow, short upper shadow.
// Direction returned: PatternBullish. You can re-weight in PatternManager if desired.
...
// Parameters
//──────────────────────────────────────────────────────────────────────────────
class HammerParams : public IPatternParams
{
public:
   ...
   // Optional context: prefer downswing before hammer (reversal bias).
   ...
   // IPatternParams
   virtual IPatternParams* Clone() const { return new HammerParams(*this); }

   virtual string ToString() const
   {
      return StringFormat(
         "HammerParams{maxBodyPctTR=%.2f, maxUpperWickPctTR=%.2f, minLowerWickToBody=%.2f, "
   ...
   virtual bool Equals(const IPatternParams* other) const
   {
      ...
      const HammerParams* o = (const HammerParams*)other;
...
// Detector
//──────────────────────────────────────────────────────────────────────────────
class HammerDetector : public AbstractPatternDetector
{
public:
   ...
   HammerDetector()
   : AbstractPatternDetector(MakeDescriptor(), new HammerParams()) {}

   HammerDetector(const HammerParams &p)
   : AbstractPatternDetector(MakeDescriptor(), &p) {}

   HammerDetector(const HammerDetector &rhs)
   : AbstractPatternDetector(rhs) {}

   virtual IPatternDetector* Clone() const { return new HammerDetector(*this); }
...
protected:
   ...
   virtual bool ValidateParams(const IPatternParams* p) const
   {
      ...
      const HammerParams* e = (const HammerParams*)p;
   ...
   // Scoring: smaller body, tiny upper wick, long lower wick, optional downswing context.
   static double ScoreHammer(const double bodyPctTR,
   ...
   {
      const HammerParams* Pptr = (const HammerParams*)m_params;
      if(Pptr==NULL) return false;
      HammerParams P = *Pptr; // dot-access
      ...
      // Score & fill
      const double score = ScoreHammer(bodyPctTR, upperWickPctTR, lowerToBody, hadDown);
