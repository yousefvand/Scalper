...
// ─────────────────────────────────────────────────────────────
// ACTIVE profile: higher trade frequency, up to 8 positions,
// looser ATR/spread gates, lower decision threshold, patterns on.
// ─────────────────────────────────────────────────────────────
...
// General
input group "=== General (ACTIVE) ===";
input string          Inp_Version                 = "1.1.0-active";
input ENUM_TIMEFRAMES Inp_Timeframe               = PERIOD_M5;
input long            Inp_Magic                   = 0;        // 0 => auto (symbol+TF)
...
input int             Inp_CooldownSeconds         = 10;     // loosened
input int             Inp_MaxPositions            = 8;      // target concurrency
input int             Inp_MaxLong                 = 8;
input int             Inp_MaxShort                = 8;
input double          Inp_MaxSpreadPts            = 25.0;   // looser than “stabilize”

// **Per-chart optional gates**
input group "=== Guards (per chart) ===";
input bool            Inp_Enable_VolatilityGate   = true;   // keep on (protect extremes)
input bool            Inp_Enable_NewsFilter       = true;
...
input int             Inp_Perf_Report_Minutes     = 30;
...
input double          Inp_EQ_SoftDD_Pct           = 8.0;    // softer than stabilize
input double          Inp_EQ_HardDD_Pct           = 16.0;
input double          Inp_EQ_MinRiskScale         = 0.35;
input double          Inp_EQ_ExposureHard_Pct     = 30.0;   // allow more exposure
input int             Inp_EQ_CheckSeconds         = 5;
input bool            Inp_EQ_Log                  = false;

input bool            Inp_EnableDrawdownProtect   = false;
input double          Inp_DD_TriggerPct           = 8.0;
...
input double          Inp_Exec_MaxSlippagePts     = 15.0;
...
// Risk
input group "=== Risk (ACTIVE) ===";
input double          Inp_Risk_RiskPercent        = 1.2;   // a bit bolder per trade
...
input double          Inp_Risk_DailyLossPct       = 4.0;   // wider daily runway
input double          Inp_Risk_DailyLossAmt       = 0.0;
input bool            Inp_Risk_EnableDailyLoss    = true;
input bool            Inp_Risk_ProfitLock         = true;
input double          Inp_Risk_ProfitLockPct      = 2.0;   // halve risk after +2% day
input bool            Inp_Risk_LimitConsLosses    = true;
input int             Inp_Risk_MaxConsLosses      = 4;     // allow longer streak
input double          Inp_Risk_MaxLotPerTrade     = 0.0;   // 0 = no extra cap

// Session
input group "=== Session ===";
input bool            Inp_Sess_EnableTimeFilter   = true;
input int             Inp_Sess_StartHour          = 5;     // trade more hours
input int             Inp_Sess_EndHour            = 22;
input string          Inp_Sess_TimezoneLabel      = "Server";
input bool            Inp_Sess_NewsBlackout       = true;
input int             Inp_Sess_PreNewsMin         = 20;    // slightly looser
input int             Inp_Sess_PostNewsMin        = 20;
input bool            Inp_Sess_CloseAtEnd         = false;
input bool            Inp_Sess_BlockNewOnFriday   = true;
input int             Inp_Sess_FridayCutoffHour   = 20;

// Signals
input group "=== Signals (ACTIVE) ===";
input bool            Inp_Sig_TradeByPatterns     = true;  // enable fallback
input bool            Inp_Sig_TradeByIndicators   = true;
input bool            Inp_Sig_EnablePatterns      = true;
input bool            Inp_Sig_UseClosedBarOnly    = false; // evaluate current bar for more trades
...
input int             Inp_Sig_PatternFusionMode   = 0; // PATT_ADD

input double          Inp_Sig_DecisionThreshold   = 0.30; // lower bar → more signals
input int             Inp_Sig_ThresholdMode       = 0;   // ABSOLUTE

input double          Inp_Sig_MinPatternStrength  = 0.40; // accept more patterns
input bool            Inp_Sig_EnableTrendFilter   = false;
input int             Inp_Sig_TrendSource         = 1; // TREND_SLOPE
input double          Inp_Sig_SlopeSensitivityDeg = 2.5;
...
input double          Inp_Sig_VWAP_MaxDistPts     = 16.0; // a bit looser
...
input double          Inp_Sig_ATR_MinPts          = 2.0;  // lower → more trades
input double          Inp_Sig_ATR_MaxPts          = 80.0; // allow higherATR sessions
...
namespace Config
{
   ...
   void SanitizeInputs()
   {
      ...
      if(Inp_Debug_LogSanitized)
      {
         ...
         LOG.Info(
            StringFormat("[%s] [config] Risk=%.2f%% dailyLoss=%.2f%% consLoss=%d lotCap=%.2f",
                         sEA.symbol, sRisk.riskPercent, sRisk.dailyLossPercent,
                         sRisk.maxConsecutiveLosses, sRisk.maxLotPerTrade));
         LOG.Info(
            StringFormat("[%s] [config] Signal thr=%.2f closedBar=%s pattMin=%.2f ATR[%.1f..%.1f]",
                         sEA.symbol, sSignal.decisionThreshold, _B(sSignal.useClosedBarOnly),
                         sSignal.minPatternStrength, sSignal.atrMinPts, sSignal.atrMaxPts));
      }
      s_ready = true;
   }
