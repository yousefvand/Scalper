...
#include "Logger.mqh"   // for Logger::Get()

// tiny bool→string helper (as requested)
inline string _B(const bool b) { return b ? "true" : "false"; }

// ─────────────────────────────────────────────────────────────
// All config structs + Inputs + Sanitizer in one file
// Strategy: Option A → DO NOT call LoadFromInputs(); we fill from inputs here,
//            apply clamps/fallbacks, then call Validate().
// Access with Config::EA(), Config::Risk(), ...
// ─────────────────────────────────────────────────────────────
...
struct EAConfig
{
   ...
   // Optional gates (per-chart toggles)
...
struct SessionConfig
{
   ...
   void Validate()
   {
      ...
      if(fridayCutoffHour < 0 || fridayCutoffHour > 23) fridayCutoffHour = 20;
   }
...
// General
input group "=== General ===";
input string          Inp_Version                 = "1.0.0";
input ENUM_TIMEFRAMES Inp_Timeframe               = PERIOD_M5;
input long            Inp_Magic                   = 0;        // 0 => auto per chart (symbol+TF)
...
input bool            Inp_NewBarOnly              = false;    // allow intra-bar signals
input bool            Inp_OneTradePerBar          = false;    // allow multiple per bar
input int             Inp_CooldownSeconds         = 0;
// ★ Lift caps
input int             Inp_MaxPositions            = 24;
input int             Inp_MaxLong                 = 24;
input int             Inp_MaxShort                = 24;
// ★ Looser spread gate (tune per broker/symbol)
input double          Inp_MaxSpreadPts            = 35.0;

// **Per-chart optional gates**
input group "=== Guards (per chart) ===";
input bool            Inp_Enable_VolatilityGate   = false;   // default OFF
input bool            Inp_Enable_NewsFilter       = false;   // default OFF
...
input bool            Inp_Perf_ToJournal          = false;
...
input double          Inp_EQ_SoftDD_Pct           = 5.0;
input double          Inp_EQ_HardDD_Pct           = 10.0;
input double          Inp_EQ_MinRiskScale         = 0.40;
input double          Inp_EQ_ExposureHard_Pct     = 15.0;
input int             Inp_EQ_CheckSeconds         = 5;
input bool            Inp_EQ_Log                  = false;

input bool            Inp_EnableDrawdownProtect   = false;
input double          Inp_DD_TriggerPct           = 5.0;
input double          Inp_DD_RecoverPct           = 2.5;
input bool            Inp_DD_PanicClose           = false;

input bool            Inp_EnableExecOptimizer     = false;
input int             Inp_Exec_MaxRetries         = 2;
input double          Inp_Exec_MaxSlippagePts     = 10.0;
input double          Inp_Exec_SpreadAdaptFactor  = 0.0;

input bool            Inp_EnableLatencyComp       = false;
input double          Inp_Latency_MaxAdjustPts    = 3.0;
input int             Inp_Latency_TickWindow      = 30;
input int             Inp_Latency_ExpectedMs      = 150;

input bool            Inp_EnableAutoReboot        = false;
input int             Inp_Reboot_ThresholdSec     = 120;
...
// Risk
input group "=== Risk ===";
input double          Inp_Risk_RiskPercent        = 1.0;   // keep sane; frequency ↑, not per-trade risk
input bool            Inp_Risk_UseEquity          = true;
input bool            Inp_Risk_EnableDDGuardian   = false;
input double          Inp_Risk_MaxDD_Pct          = 0.0;
input bool            Inp_Risk_CloseAllOnDD       = true;
input double          Inp_Risk_DailyLossPct       = 0.0;
input double          Inp_Risk_DailyLossAmt       = 0.0;
input bool            Inp_Risk_EnableDailyLoss    = false;
input bool            Inp_Risk_ProfitLock         = false;
input double          Inp_Risk_ProfitLockPct      = 0.0;
input bool            Inp_Risk_LimitConsLosses    = false;
input int             Inp_Risk_MaxConsLosses      = 0;
...
input int             Inp_Sess_EndHour            = 23;
input string          Inp_Sess_TimezoneLabel      = "Server";
// ★ keep news off by default; if enabled, shrink windows
input bool            Inp_Sess_NewsBlackout       = false;
input int             Inp_Sess_PreNewsMin         = 10;    // was 30
input int             Inp_Sess_PostNewsMin        = 10;    // was 30
input bool            Inp_Sess_CloseAtEnd         = false;
// ★ do not block Friday by default (more trades)
input bool            Inp_Sess_BlockNewOnFriday   = false; // was true
input int             Inp_Sess_FridayCutoffHour   = 20;

// Signals
input group "=== Signals ===";
// ★ enable both pipelines
input bool            Inp_Sig_TradeByPatterns     = true;   // was false
input bool            Inp_Sig_TradeByIndicators   = true;
input bool            Inp_Sig_EnablePatterns      = true;
// ★ intra-bar decisions allowed
input bool            Inp_Sig_UseClosedBarOnly    = false;
...
input double          Inp_Sig_wPatternNeutral     = 0.10;
input int             Inp_Sig_PatternFusionMode   = 0; // PATT_ADD..PATT_NONE

// ★ lower decision bar to catch more setups
input double          Inp_Sig_DecisionThreshold   = 0.25;  // was 0.30
input int             Inp_Sig_ThresholdMode       = 0; // THRESH_ABSOLUTE/STRICT

input double          Inp_Sig_MinPatternStrength  = 0.50;
input bool            Inp_Sig_EnableTrendFilter   = false;
input int             Inp_Sig_TrendSource         = 1; // TREND_SLOPE
input double          Inp_Sig_SlopeSensitivityDeg = 3.0;
input int             Inp_Sig_BarsLookback        = 50;
input bool            Inp_Sig_UseRsiMidline       = true;
input int             Inp_Sig_RsiMid              = 50;

input int             Inp_Sig_MinCandlePoints     = 0;
input double          Inp_Sig_MinBodyToRange      = 0.0;
input double          Inp_Sig_MaxUpperWickRatio   = -1.0;
input double          Inp_Sig_MaxLowerWickRatio   = -1.0;

input bool            Inp_Sig_EnableMTFConfirm    = false;
input ENUM_TIMEFRAMES Inp_Sig_MTF_Timeframe       = PERIOD_M5;
input int             Inp_Sig_MTF_BarsLookback    = 20;

input bool            Inp_Sig_EnableVWAPFilter    = false;
input double          Inp_Sig_VWAP_MaxDistPts     = 12.0;

input bool            Inp_Sig_EnableDivergence    = false;
input int             Inp_Sig_DivergenceLookback  = 60;

// ★ faster RSI; MA fast/slow OK; ATR lower floor
input int             Inp_Sig_MA_Fast             = 9;
input int             Inp_Sig_MA_Slow             = 21;
input ENUM_MA_METHOD  Inp_Sig_MA_Method           = MODE_EMA;
input ENUM_APPLIED_PRICE Inp_Sig_MA_Applied       = PRICE_CLOSE;
input ENUM_TIMEFRAMES Inp_Sig_MA_Timeframe        = PERIOD_CURRENT;

input int             Inp_Sig_RSI_Period          = 3;      // was 14
input ENUM_APPLIED_PRICE Inp_Sig_RSI_Applied      = PRICE_CLOSE;
input ENUM_TIMEFRAMES Inp_Sig_RSI_Timeframe       = PERIOD_CURRENT;

input int             Inp_Sig_ATR_Period          = 14;
input ENUM_TIMEFRAMES Inp_Sig_ATR_Timeframe       = PERIOD_CURRENT;
input double          Inp_Sig_ATR_MinPts          = 1.5;    // was 2.0
input double          Inp_Sig_ATR_MaxPts          = 20.0;
...
input int             Inp_Dsp_RefreshMs           = 500;
...
// Patterns (keep defaults unless expanded later)
...
namespace Config
{
   ...
   // simple per-chart magic helper when Inp_Magic==0
   ...
   long _AutoMagic_(const string sym, const ENUM_TIMEFRAMES tf)
   {
      ...
      uint h = _Hash16_(key);
      // keep in a friendly range not to collide with manual magics
   ...
   void SanitizeInputs()
   {
      ...
      // Broker/symbol caps (placeholders; populate elsewhere if needed)
      sEA.minLot        = 0.0;
      sEA.maxLot        = 0.0;
      sEA.lotStep       = 0.0;
      sEA.volDigits     = 2;
      ...
      // -------- Risk (explicit fallback rule) --------
      double rp = Inp_Risk_RiskPercent;
      if(rp < 0.0 || rp > 5.0) rp = 2.0;     // requested policy
      ...
      // -------- Patterns (defaults unless expanded later) --------
      if(Inp_Patt_EnableDefaults)
      {
         ...
         sPattern.hangingMan=true; sPattern.invertedHammer=true; sPattern.marubozu=false; sPattern.shootingStar=true; sPattern.spinningTop=false;
      ...
      {
         LOG.Info(
            StringFormat("[%s] [config] EA tf=%d magic=%I64d buy=%s sell=%s mktOnly=%s pending=%s maxSpread=%.2f",
                         sEA.symbol, (int)sEA.timeframe, sEA.magic,
                         _B(sEA.allowBuy), _B(sEA.allowSell),
                         _B(sEA.marketOnly), _B(sEA.allowPendingOrders),
                         sEA.maxSpreadPts));

         LOG.Info(
            StringFormat("[%s] [config] Guards volGate=%s newsFilter=%s",
                         sEA.symbol, _B(sEA.enableVolatilityGate), _B(sEA.enableNewsFilter)));

         LOG.Info(
            StringFormat("[%s] [config] Risk pct=%.3f useEq=%s DDG=%s maxDD=%.2f%%",
                         sEA.symbol,
                         sRisk.riskPercent, _B(sRisk.useEquityForRisk),
                         _B(sRisk.enableDrawdownGuardian), sRisk.maxDrawdownPercent));

         LOG.Info(
            StringFormat("[%s] [config] Session enabled=%s %02d-%02d news=%s pre=%d post=%d FriBlock=%s@%02d",
                         sEA.symbol,
                         _B(sSession.enableTimeFilter), sSession.startHour, sSession.endHour,
                         _B(sSession.newsBlackout), sSession.preNewsMinutes, sSession.postNewsMinutes,
                         _B(sSession.blockNewOnFriday), sSession.fridayCutoffHour));

         LOG.Info(
            StringFormat("[%s] [config] Signal EMA=%.2f SMA=%.2f thr=%.2f MTF=%s",
                         sEA.symbol,
                         sSignal.wEMA, sSignal.wSMA, sSignal.decisionThreshold,
                         _B(sSignal.enableMTFConfirm)));

         LOG.Info(
            StringFormat("[%s] [config] Display HUD=%s markers=%s refresh=%dms theme=%s",
                         sEA.symbol,
                         _B(sDisplay.showHUD), _B(sDisplay.showTradeMarkers),
                         sDisplay.refreshMs, _B(sDisplay.darkTheme)));
      }
      s_ready = true;
   }
