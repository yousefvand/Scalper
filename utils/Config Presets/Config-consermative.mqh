...
input string          Inp_Version                 = "1.2.0-conservative";
...
input int             Inp_CooldownSeconds         = 45;     // slower cadence
input int             Inp_MaxPositions            = 4;      // cap concurrency
input int             Inp_MaxLong                 = 4;
input int             Inp_MaxShort                = 4;
input double          Inp_MaxSpreadPts            = 15.0;   // tighter spread gate

// **Per-chart optional gates**
input group "=== Guards (per chart) ===";
input bool            Inp_Enable_VolatilityGate   = true;   // protect extremes
...
input double          Inp_EQ_MinRiskScale         = 0.30;
input double          Inp_EQ_ExposureHard_Pct     = 20.0;
...
input double          Inp_Exec_MaxSlippagePts     = 8.0;    // conservative
...
input double          Inp_Risk_RiskPercent        = 0.60;  // per trade
input bool            Inp_Risk_UseEquity          = true;
input bool            Inp_Risk_EnableDDGuardian   = true;
input double          Inp_Risk_MaxDD_Pct          = 12.0;  // hard stop for account (guardian)
input bool            Inp_Risk_CloseAllOnDD       = true;
input double          Inp_Risk_DailyLossPct       = 2.0;   // stop day after -2%
input double          Inp_Risk_DailyLossAmt       = 0.0;
input bool            Inp_Risk_EnableDailyLoss    = true;
input bool            Inp_Risk_ProfitLock         = true;
input double          Inp_Risk_ProfitLockPct      = 1.0;   // halve risk once +1% day
input bool            Inp_Risk_LimitConsLosses    = true;
input int             Inp_Risk_MaxConsLosses      = 3;
input double          Inp_Risk_MaxLotPerTrade     = 0.0;   // keep to broker caps

// Session
input group "=== Session (CONSERVATIVE) ===";
input bool            Inp_Sess_EnableTimeFilter   = true;
input int             Inp_Sess_StartHour          = 7;     // avoid thin Asia tail
input int             Inp_Sess_EndHour            = 20;    // avoid late NY chop
input string          Inp_Sess_TimezoneLabel      = "Server";
input bool            Inp_Sess_NewsBlackout       = true;
input int             Inp_Sess_PreNewsMin         = 30;
input int             Inp_Sess_PostNewsMin        = 30;
input bool            Inp_Sess_CloseAtEnd         = false;
input bool            Inp_Sess_BlockNewOnFriday   = true;
input int             Inp_Sess_FridayCutoffHour   = 18;
...
input bool            Inp_Sig_UseClosedBarOnly    = true;  // wait for bar close
...
input double          Inp_Sig_wPatternDirectional = 0.25;  // slightly toned down
input double          Inp_Sig_wPatternNeutral     = 0.10;
input int             Inp_Sig_PatternFusionMode   = 0; // PATT_ADD

input double          Inp_Sig_DecisionThreshold   = 0.45;  // higher bar → fewer trades
input int             Inp_Sig_ThresholdMode       = 0;     // ABSOLUTE

input double          Inp_Sig_MinPatternStrength  = 0.55;  // require stronger patterns
input bool            Inp_Sig_EnableTrendFilter   = true;  // align with broader trend
input int             Inp_Sig_TrendSource         = 2;     // TREND_MA
input double          Inp_Sig_SlopeSensitivityDeg = 3.0;
input int             Inp_Sig_BarsLookback        = 80;
...
input bool            Inp_Sig_EnableMTFConfirm    = false; // can turn on later
input ENUM_TIMEFRAMES Inp_Sig_MTF_Timeframe       = PERIOD_M15;
input int             Inp_Sig_MTF_BarsLookback    = 20;

input bool            Inp_Sig_EnableVWAPFilter    = true;  // keep entries near mean
...
input int             Inp_Sig_RSI_Period          = 3;     // works with BnB
...
input double          Inp_Sig_ATR_MinPts          = 3.0;   // require some range
input double          Inp_Sig_ATR_MaxPts          = 60.0;  // avoid extreme chaos
...
namespace Config
{
   ...
   void SanitizeInputs()
   {
      ...
      if(Inp_Debug_LogSanitized)
      {
         ...
         Logger::Get().Info(
            StringFormat("[%s] [config] Signal thr=%.2f closedBar=%s pattMin=%.2f ATR[%.1f..%.1f] VWAP<=%.1f",
